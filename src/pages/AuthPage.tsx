...
const AuthPage = () => {
  ...
  useEffect(() => {
    if (user && hasExistingReport !== null) {
      ...
      // Check if we're coming from anonymous results and need to convert
      if (location.state?.fromAnonymous && isSignUp) {
        convertAnonymousReportToUserReport(user.id).then(() => {
